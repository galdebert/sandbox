...
public:
	...
	template <typename T> Object(const T& data) { interface_ = make_shared<Concrete<T> >(data); }
...
private:
	...
	template <typename T> struct Concrete : public Interface
	{
		...
		virtual shared_ptr<Interface> Clone() const override { return make_shared<Concrete<T> >(data_); }
