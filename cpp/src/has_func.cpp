...
template<typename T>
typename std::enable_if<std::is_constructible<T, Allocator&>::value>::type
construct_value(T*)
{
	std::cout << "construct with allocator\n";
}
...
construct_value(T*)
{
	std::cout << "construct without allocator\n";
}

// in Stingray
// template <class FIRST, class SECOND, bool firstaware, bool secondaware> struct Pair {}
template<typename T, int Tallocaware>
struct TypeAndAwareness
{
};
